...
void ListMain()
{
	...
	//it = container.begin() + 3; //컴파일오류. 리스트는 다음노드만 접근가능하다.
...
void DequeMain()
{
	deque<int> container(10);//컨테이너생성시 크기를 지정가능하다.
	container[0] = 10;
	for (int i = 0; i < container.size(); i++)
		container[i] = (container.size() - i) * 10;
	cout << "Print:";
	...
	it = container.begin() + 3; //삽입할 이터레이터 설정
	deque<int>::iterator itInsert = container.insert(it, -20); //해당 이터레이터에 앞에 값을 삽입함.
	deque<int>::iterator itFind = find(container.begin(), container.end(), 1000); //삭제할 이터레이터 설정
	if (itFind != container.end()) //마지막값은 존제하지않는다.
		cout << "itFind:" << *itFind << endl;
	container.erase(itInsert); //해당 이터레이터를 배열에서 삭제한다.
	cout << "PrintPtr:";
...
//맵: 사전식으로 데이터를 찾을수있다.
//해당영어단어를 넣으면 한국어 결과가 나온다.//균형(레드블랙)이진트리
void MapMain()
{
...
//셋: 순서없이 데이터를 넣는다. 데이터는 순서와 상관없이 데이터를 찾는다.//균형(레드블랙)이진트리
void SetMain()
{
	set<int> setData;
